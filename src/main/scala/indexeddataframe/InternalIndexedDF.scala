...
class InternalIndexedDF {
  ...
  /** function that sets the schema given a list of data types and also stores the index column
    * @param types
    * @param columnNo
    */
  ...
  /** function that packs the batchNo, rowId and offset of a row into a 64 bit number example:
    * @param batchNo
    *   -> the batch id
    * @param offset
    *   -> the offset within a batch
    * @param size
    *   -> the size of the row
    * @return
    *   the packed 64 bit integer
    */
  ...
  /** function that unpacks the batchNo, rowId and offset of a row from a 64 bit number example:
    * @param batchNo
    *   -> id of the rowbatch
    * @param offset
    *   -> offset within rowbatch
    * @param size
    *   -> size of row
    * @return
    *   the unpacked integer as a tuple of (batchNo, offset, size)
    */
  ...
  def appendRow(row: InternalRow) = {
    ...
    val key = schema(indexCol).dataType match {
      ...
      case StringType =>
        Murmur3_x86_32.hashUnsafeBytes(unsafeRow.getString(indexCol).getBytes(), Platform.BYTE_ARRAY_OFFSET, unsafeRow.getString(indexCol).length, 42)
  ...
  /** iterator returned by the get function on the indexed data frame
    * @rowPointer:
    *   parameter that contains the #prev row pointer
    */
  ...
  /** function that performs lookups in the indexed data frame returns an iterator of rows
    */
  def get(key: Any): Iterator[InternalRow] = {
    ...
    val internalKey = key match {
      ...
      // if the key is a string, just get the bytes and hash them
      case _: String =>
        Murmur3_x86_32.hashUnsafeBytes(key.asInstanceOf[String].getBytes(), Platform.BYTE_ARRAY_OFFSET, key.asInstanceOf[String].length, 42)
  ...
  /** a similar multiget, but this one returns joined rows, composed of left + right joined rows we need the projection as a parameter to convert back
    * to unsafe rows
    * @param keys
    * @return
    */
  ...
  /** function that returns a snapshot of the InternalIndexedDF partition but makes a snapshot of the CTries (such that the new updates are not
    * reflected in the old data structure) this function is efficient because there is no data copying, we use the cTrie to do all the smart
    * heavy-lifting with the snapshotting capability
    */
